...
用法:
  ...
  # 只处理某一个 mix 子目录
  python collect_sa_results.py --mix-dir ./experiment_results_sa/mix_0.0_14.0_topk_200

  # 手动指定参数（单目录，兼容旧用法）
  python collect_sa_results.py --eps_low 0 --eps_high 14 --top_k 200 --seeds 42,43
"""
...
def extract_test_acc(log_file: Path) -> Optional[float]:
    try:
        ...
        print(f"  读取失败 {log_file}: {exc}")
        return None


def parse_mix_dir_name(name: str) -> Optional[Tuple[float, float, int]]:
    matched = MIX_DIR_RE.match(name)
    if not matched:
        return None
    return float(matched.group(1)), float(matched.group(2)), int(matched.group(3))
...
def infer_from_logs(mix_dir: Path) -> Tuple[List[float], List[int], List[int]]:
    ...
    for filepath in mix_dir.glob(LOG_GLOB):
        matched = LOG_NAME_RE.match(filepath.name)
        if not matched:
            continue
        eps_set.add(float(matched.group("eps")))
        seed_set.add(int(matched.group("seed")))
        topk_set.add(int(matched.group("topk")))
...
def collect_one_mix_dir(
    ...
    if seeds is None:
        seeds = inferred_seeds if inferred_seeds else list(range(42, 47))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="收集 QNLI SA 实验结果（格式与 experiment_results/collect_results.py 一致）"
    )
    parser.add_argument(
        "--base_result_dir",
        type=str,
    ...
    if args.mix_dir:
        ...
        top_k_val = args.top_k if args.top_k is not None else 200
        mix_dirs = [
            Path(args.base_result_dir).resolve()
            / f"mix_{args.eps_low}_{args.eps_high}_topk_{top_k_val}"
        ]
    else:
    ...
    all_parts = []
    for directory in mix_dirs:
        if not directory.is_dir():
            print(f"  [跳过] 不存在: {directory}")
            continue
        parsed = parse_mix_dir_name(directory.name)
        top_k_val = args.top_k if args.top_k is not None else (parsed[2] if parsed else 200)
        df = collect_one_mix_dir(directory, eps_values=eps_values, seeds=seeds, top_k=top_k_val)
        if df is not None:
            df = df.copy()
            df["mix_dir"] = directory.name
            all_parts.append(df)
