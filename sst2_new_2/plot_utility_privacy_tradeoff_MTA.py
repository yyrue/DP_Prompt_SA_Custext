...
# -*- coding: utf-8 -*-
"""
Plot utility-privacy tradeoff for mask token attack.
...
def load_custext_tradeoff(utility_csv: Path, privacy_csv: Path) -> pd.DataFrame:
    ...
    required_privacy_v1 = {"eps", "defense_rate_mean", "defense_rate_std"}
    required_privacy_v2 = {"eps", "平均值", "标准差"}
    ...
    if required_privacy_v1.issubset(privacy_df.columns):
        pass
    elif required_privacy_v2.issubset(privacy_df.columns):
        privacy_df = privacy_df.rename(columns={"平均值": "defense_rate_mean", "标准差": "defense_rate_std"})
    else:
        raise ValueError(
            f"{privacy_csv} missing columns, expected one of: "
            f"{required_privacy_v1} or {required_privacy_v2}"
        )
    ...
    privacy_df["defense_rate_mean"] = _to_numeric(privacy_df["defense_rate_mean"])
    privacy_df["defense_rate_std"] = _to_numeric(privacy_df["defense_rate_std"]).fillna(0.0)
    privacy_df = privacy_df.dropna(subset=["eps", "defense_rate_mean"])
...
def load_mixed_tradeoff(utility_csv: Path, privacy_csv: Path) -> pd.DataFrame:
    ...
    required_privacy = {"mix_tag", "eps_prime", "defense_rate_mean", "defense_rate_std"}
    if not required_utility.issubset(utility_df.columns):
        raise ValueError(f"{utility_csv} missing columns: {required_utility - set(utility_df.columns)}")
    ...
    privacy_df = privacy_df.rename(columns={"eps_prime": "eps"})
    privacy_df["eps"] = _to_numeric(privacy_df["eps"])
    privacy_df["defense_rate_mean"] = _to_numeric(privacy_df["defense_rate_mean"])
    privacy_df["defense_rate_std"] = _to_numeric(privacy_df["defense_rate_std"]).fillna(0.0)
    privacy_df["mix_base"] = privacy_df["mix_tag"].map(_extract_mix_base)
    privacy_df = privacy_df.dropna(subset=["eps", "defense_rate_mean"])
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Plot utility-privacy tradeoff curve(s) for mask token attack"
    )
    parser.add_argument(
        "--custext-utility-csv",
        type=str,
    ...
    parser.add_argument(
        ...
        help="CuSText privacy CSV (defense rate over eps)",
    )
    parser.add_argument(
        "--mixed-privacy-csv",
        type=str,
        default=str(root / "attack_results_mixed" / "attack_summary_all_mixes_by_eps_prime.csv"),
        help="Mixed privacy CSV (defense rate over eps_prime)",
    )
    parser.add_argument(
        "--out",
        type=str,
        default=str(root / "attack_results" / "utility_privacy_tradeoff_mask_token_attack.png"),
    ...
    ax.set_title("Utility-Privacy Tradeoff (Mask Token Attack)")
