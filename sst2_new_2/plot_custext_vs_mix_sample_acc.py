...
- CusText：experiment_results/results_statistics_topk_200.csv（不绘制 eps=3）
- Baseline（no sanitized）：experiment_results/results_baseline_statistics.csv（eps=inf）
- Mix：experiment_results_sa/mix_*/results_statistics_topk_200.csv（可多组）
...
def discover_mix_csvs(mixed_root: Path) -> List[Path]:
    paths = sorted(mixed_root.glob("mix_*/results_statistics_topk_200.csv"))
...
def main():
    ...
    if len(df_cf) > 0:
        ...
        finite_eps_ticks.extend(float(v) for v in eps if np.isfinite(v))
    ...
    for i, csv_path in enumerate(mix_paths):
        ...
        if len(xp):
            finite_eps_ticks.extend(float(v) for v in xp if np.isfinite(v))
    ...
    ax.set_title("Utility Comparison:Custext Top200 vs Sample Amplification - SST2", fontsize=14)
    ...
    tick_pos = [float(t) for t in DEFAULT_FINITE_XTICKS if t <= max_x + 1e-9]
    extra_tick_pos = sorted(
        {
            float(t)
            for t in finite_eps_ticks
            if t > DEFAULT_FINITE_XTICKS[-1] + 1e-9 and t <= max_x + 1e-9
        }
    )
    for t in extra_tick_pos:
        if not any(np.isclose(t, existing, rtol=0, atol=1e-9) for existing in tick_pos):
            tick_pos.append(t)
    tick_lbl = [_fmt_eps_num(t) for t in tick_pos]
