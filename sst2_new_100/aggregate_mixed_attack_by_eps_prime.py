...
def aggregate_one_mix_dir(mix_dir: Path) -> Optional[pd.DataFrame]:
    ...
    g["attack_accuracy_std"] = g["attack_accuracy_std"].fillna(0.0)
    g["defense_rate_mean"] = 1.0 - g["attack_accuracy_mean"]
    g["defense_rate_std"] = g["attack_accuracy_std"]

    # 元信息（各文件应一致，取首行）
    for col in ("eps_low", "eps_high", "mix_tag"):
        if col in all_df.columns:
            g[col] = all_df[col].iloc[0]

    out_path = mix_dir / OUT_NAME
    g.to_csv(out_path, index=False)
    print(
        f"  [{mix_dir.name}] 写入 {out_path}（{len(frames)} 个 seed 汇总 → {len(g)} 行 eps_prime）"
    )
    return g
