...
class Trainer:
    ...
    def evaluate(self, data_loader):
        ...
        preds = np.argmax(y_hat_list, axis=1).flatten() 
        labels = y_list.flatten()
        acc = np.sum(preds==labels) / len(y_list)
        return acc
    
    def predict(self,data_loader):
        self.model.load_state_dict(torch.load(f'{args.save_path}/{args.embedding_type}_{args.mapping_strategy}_{args.privatization_strategy}_eps_{args.eps}_top_{args.top_k}_save_{args.save_stop_words}_seed_{args.seed}_model.pkl'))
        self.model.eval()
        y_list = []
        y_hat_list = []
        for batch in tqdm(data_loader):
    ...
    def save_model(self):
        if not os.path.exists(args.save_path):
            os.mkdir(args.save_path)
        torch.save(self.model.state_dict(),f'{args.save_path}/{args.embedding_type}_{args.mapping_strategy}_{args.privatization_strategy}_eps_{args.eps}_top_{args.top_k}_save_{args.save_stop_words}_seed_{args.seed}_model.pkl')
